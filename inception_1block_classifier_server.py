...
class Detection_net(nn.Module):
	...
	def run_code_for_testing(self, net, images):
		...
		with torch.no_grad():
			...
			prediction = {}
			prediction['label'] = predicted.cpu().item()
			prediction['score'] = soft_output.data[0][predicted.cpu().item()].cpu().item()
			print(prediction)
			return prediction
...
def main():
	...
	while True:
		...
		jsonpred = detnet.run_code_for_testing(net, image)
		socket.send_json(jsonpred)
		#socket.send((jsonpred.encode('utf-8')))
